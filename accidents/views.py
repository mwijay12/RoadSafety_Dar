...
def api_monthly_report(_request):
    ...
    # Use the management command logic but build inline
    ...
    qs = Accident.objects.filter(occurred_at__gte=period_start, occurred_at__lte=period_end)
    total = qs.count()

    # Build response
    response = HttpResponse(content_type="application/pdf")
    response["Content-Disposition"] = (
        f'attachment; filename="roadsafety_{period_start:%Y_%m}.pdf"'
    )
    ...
    story.append(Paragraph(
        f"Road Safety Dar es Salaam — {period_start:%B %Y}",
        title_style,
    ))
    ...
    if total == 0:
        story.append(Paragraph(
            "<b>No incidents recorded in this period.</b>",
            styles["Normal"],
        ))
    else:
        fatal = qs.filter(severity="fatal").count()
        total_fatalities = sum(qs.values_list("fatalities", flat=True))
        total_casualties = sum(qs.values_list("casualties", flat=True))

        kpi = [
            ["Metric", "Value"],
            ["Total Incidents", str(total)],
            ["Fatal Incidents", str(fatal)],
            ["Total Deaths", str(total_fatalities)],
            ["Total Casualties", str(total_casualties)],
        ]
        t = Table(kpi, colWidths=[10*cm, 5*cm])
    ...
    story.append(Spacer(1, 0.5*cm))
    story.append(Paragraph(
        f"<i>Generated by RoadSafety_Dar v1.2 on {timezone.now():%Y-%m-%d %H:%M} EAT</i>",
        styles["Italic"],
    ))
...
def api_telegram_webhook(request):
    ...
    Configure via: https://api.telegram.org/bot<TOKEN>/setWebhook?url=https://yourdomain.com/api/telegram/webhook/
    """
    if request.method == "POST":
        ...
        if text == "/start" and chat_id:
            ...
            send_message(chat_id, (
                ...
                f"Fatal: {fatal}\n"
                f"Last 24h: {Accident.objects.filter(occurred_at__gte=timezone.now()-timedelta(days=1)).count()}\n"
                f"\n🔗 https://roadsafety.co.tz/dashboard/"
            ))
        elif text == "/hotspots" and chat_id:
            from accidents.telegram_bot import send_message
            top = _build_recommendation_engine_context()["junction_buckets"] if _build_recommendation_engine_context() else {}
